...
class RAML10Importer extends RAMLImporter {
	...
	mapRequestBody(methodBody, checkEmptyType, mimeType) {
		...
		if (methodBody.properties && !_.isEmpty(methodBody.properties)) {
			...
			data.body = this._mapSchema(methodBody, false, false);
		...
		for (const paramName in source.properties) {
			...
			if (RAML10Importer.isArray(param)) {
				...
				if (!_.isArray(param.type) && typeof param.type === 'object') {
					param.type = this._mapSchema(param.type, false, false);
					param = this.mapNestedType(param);
				}
			...
			if (param.hasOwnProperty('scalarsAnnotations') && param.scalarsAnnotations.hasOwnProperty('required')) {
				const annotations = _.reduce(param.scalarsAnnotations.required, function (obj, prop) {
