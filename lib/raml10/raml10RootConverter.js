...
class Raml10RootConverter extends Converter {
	...
	_export(model) {
		const attrIdMap = {};
		const attrIdSkip = ['info', 'baseUri', 'baseUriParameters', 'mediaType', 'protocols','securityDefinitions','resources', 'types', 'resourceTypes', 'annotations', 'tags', 'externalDocs'];
		const ramlDef = Raml10RootConverter.copyObjectFrom(model, attrIdMap, attrIdSkip);

		if (model.hasOwnProperty('info')) {
			const infoConverter = new Raml10InfoConverter(this.model);
		...
		if (model.hasOwnProperty('(oas-responses)')) {
			const value = model['(oas-responses)'];
			if (value.hasOwnProperty('headers') && !_.isEmpty(value.headers)) {
				const definitionConverter = new Raml10DefinitionConverter();
				for (const id in value.headers) {
					const header = value.headers[id];
					value.headers[id] = definitionConverter._import(header);
				}
			}
		}
