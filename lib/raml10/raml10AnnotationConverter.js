...
class Raml10AnnotationConverter extends Converter {
	...
	exportAnnotation(ramlDef, value) {
		...
		Raml10CustomAnnotationConverter._createAnnotationType(this.ramlDef, this.annotationPrefix, value.name, value.definition);
	}
	...
	_import(ramlDef) {
		...
		for (const id in ramlDef.annotations) {
			...
			const name = _.isArray(ramlDef.annotations) ? ramlDef.annotations[id].name : id;
			const value = _.isArray(ramlDef.annotations) ? ramlDef.annotations[id].definition : ramlDef.annotations[id].structuredValue;
