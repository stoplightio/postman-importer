...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
			...
			if (_.isArray(model.responses) && !_.isEmpty(model.responses)) {
				...
				for (const index in model.responses) {
					...
					if (val.hasOwnProperty('httpStatusCode') && !val.hasOwnProperty('reference')) {
						...
						const body = Raml10MethodConverter.exportBodies(val, definitionConverter, mimeTypes, this.model, this.annotationPrefix, this.ramlDef);
						...
						Raml10RootConverter.exportAnnotations(this.model, this.annotationPrefix, this.ramlDef, val, response);
		...
		if (model.hasOwnProperty('headers')) {
			if (_.isArray(model.headers) && !_.isEmpty(model.headers)) {
				const parameterConverter = new ParameterConverter(this.model, null, this.annotationPrefix, this.ramlDef);
		...
		if (model.hasOwnProperty('parameters')) {
			if (_.isArray(model.parameters) && !_.isEmpty(model.parameters)) {
				const parameterConverter = new ParameterConverter(this.model, 'query', this.annotationPrefix, this.ramlDef);
		...
		const mimeTypes = !_.isEmpty(internalMimeTypes) ? internalMimeTypes : globalMimeTypes;
		const body = Raml10MethodConverter.exportBodies(model, definitionConverter, mimeTypes, this.model, this.annotationPrefix, this.ramlDef);
		...
		if (object.hasOwnProperty('formBodies')) {
			if (_.isArray(object.formBodies) && !_.isEmpty(object.formBodies)) {
				for (const index in object.formBodies) {
					...
					const mimeType = !_.isEmpty(mimeTypes) && helper.getValidFormDataMimeTypes.includes(mimeTypes[0]) ? mimeTypes[0] : 'multipart/form-data';
					const result = converter._export(val.definition);
					if (val.hasOwnProperty('description')) result.description = val.description;
					Raml10MethodConverter.exportRequired(val, result);
					// Raml10RootConverter.exportAnnotations(val, result);
					if (!body[mimeType]) {
						body[mimeType] = {};
						body[mimeType].properties = {};
					}
					body[mimeType].properties[val.definition.name] = result;
				}
			}
		}
		return body;
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['responses', 'description', 'headers', 'body', 'queryParameters', 'queryString', 'name', 'usage', 'is', 'securedBy', 'annotations', 'protocols'];
		const model = Raml10MethodConverter.copyObjectFrom(ramlDef, attrIdMap, attrIdSkip);
		const definitionConverter = new Raml10DefinitionConverter(null, null, this.ramlDef);
		...
		if (ramlDef.hasOwnProperty('responses')) {
			...
			if (_.isArray(ramlDef.responses)) {
				...
				for (const id in ramlDef.responses) {
					...
					if (!_.isEmpty(headers)) response.headers = headers;
					const bodies = Raml10MethodConverter.importBodies(value, definitionConverter, null, isRaml08Version);
	...
	static importBodies(object, converter, model, isRaml08Version) {
		...
		if (object.hasOwnProperty('body')) {
			for (const id in object.body) {
				...
				if (isRaml08Version) Raml10MethodConverter.importRaml08Schema(value, body, converter);
				else {
					if (_.isArray(schema)) {
						schema = { type: schema };
				...
				if (body.definition.hasOwnProperty('definitions')) {
	...
	static importRaml08Schema(source, target, converter) {
		...
		if (source.hasOwnProperty('schema')) {
			...
			if (isReference && target.definition.type) {
				target.definition.reference = target.definition.type;
				delete target.definition.type;
			}
			if (source.hasOwnProperty('example')) {
				target.definition.example = source.example;
			}
