...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['responses', 'headers', 'bodies', 'method', 'parameters', 'consumes'];
		...
		if (model.hasOwnProperty('headers')) {
			if (_.isArray(model.headers) && !_.isEmpty(model.headers)) {
				...
				for (const index in model.headers) {
					...
					Raml10MethodConverter.exportRequired(headers[val.name]);
				}
		...
		if (model.hasOwnProperty('parameters')) {
			if (_.isArray(model.parameters) && !_.isEmpty(model.parameters)) {
				const parameters = {};
				for (const index in model.parameters) {
					if (!model.parameters.hasOwnProperty(index)) continue;
					
					const val = model.parameters[index];
					parameters[val.name] = raml10DefinitionConverter._export(val.definition);
					Raml10MethodConverter.exportRequired(parameters[val.name]);
				}
				ramlDef.queryParameters = parameters;
			}
		}
		...
		if (object.hasOwnProperty('bodies')) {
			if (_.isArray(object.bodies) && !_.isEmpty(object.bodies)) {
				for (const index in object.bodies) {
					...
					Raml10MethodConverter.exportRequired(body[mimeType]);
				}
			}
		}
		return body;
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['responses', 'headers', 'body', 'queryParameters'];
		...
		if (ramlDef.hasOwnProperty('headers')) {
			...
			for (const id in ramlDef.headers) {
				...
				Raml10MethodConverter.importRequired(header.definition);
				headers.push(header);
			}
		...
		if (ramlDef.hasOwnProperty('queryParameters')) {
			...
			for (const id in ramlDef.queryParameters) {
				...
				Raml10MethodConverter.importRequired(parameter.definition);
	...
	static importBodies(object, converter) {
		...
		if (object.hasOwnProperty('body')) {
			for (const id in object.body) {
				...
				body.definition.required = value.hasOwnProperty('required') ? value.required : true;
				bodies.push(body);
			}
		}
		return bodies;
