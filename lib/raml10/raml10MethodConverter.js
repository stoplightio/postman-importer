...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['responses', 'headers', 'bodies', 'formBodies', 'method', 'parameters', 'queryStrings', 'consumes', 'usage', 'path', 'produces', 'securedBy', 'annotations', 'tags', 'summary', 'deprecated', 'externalDocs'];
			...
			if (_.isArray(model.responses) && !_.isEmpty(model.responses)) {
				...
				for (const index in model.responses) {
					...
					if (val.hasOwnProperty('httpStatusCode') && !val.hasOwnProperty('reference')) {
						...
						const body = Raml10MethodConverter.exportBodies(val, definitionConverter, this.model, this.annotationPrefix, this.ramlDef);
						const bodyDef = body[Object.keys(body)[0]];
						if (bodyDef && bodyDef.hasOwnProperty('examples') && (typeof bodyDef.examples === 'string' || _.isArray(bodyDef.examples))) {
							let example = [];
							if (bodyDef.hasOwnProperty('example')) {
								if (_.isArray(bodyDef.example)) {
									example = _.concat(example, bodyDef.example);
								} else {
									example.push(bodyDef.example);
								}
							}
							if (_.isArray(bodyDef.examples)) {
								bodyDef.examples.map(val => {example = _.concat(example, val)});
							} else {
								example.push(bodyDef.examples);
							}
							bodyDef.example = example;
		...
		const body = Raml10MethodConverter.exportBodies(model, definitionConverter, this.model, this.annotationPrefix, this.ramlDef);
		...
		if (model.hasOwnProperty('deprecated')) {
	...
	static exportBodies(object, converter, model, annotationPrefix, ramlDef) {
		const body = {};
