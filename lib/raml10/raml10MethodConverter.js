...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['responses', 'headers', 'bodies', 'formBodies', 'method', 'parameters', 'consumes', 'usage', 'path', 'produces', 'securedBy', 'annotations', 'tags', 'summary', 'externalDocs'];
		...
		if (model.hasOwnProperty('responses')) {
			if (_.isArray(model.responses) && !_.isEmpty(model.responses)) {
				...
				for (const index in model.responses) {
					...
					if (val.hasOwnProperty('httpStatusCode') && !inheritedResponses.includes(helper.getResponseName(model.method, val.httpStatusCode)) &&
		...
		if (model.hasOwnProperty('headers')) {
			if (_.isArray(model.headers) && !_.isEmpty(model.headers)) {
				const parameterConverter = new ParameterConverter(this.model, this.parentResource, traitModels, inheritedHeaders);
		...
		if (model.hasOwnProperty('parameters')) {
			if (_.isArray(model.parameters) && !_.isEmpty(model.parameters)) {
				const parameterConverter = new ParameterConverter(this.model, this.parentResource, traitModels, inheritedParams, 'query');
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['responses', 'headers', 'body', 'queryParameters', 'name', 'usage', 'is', 'securedBy', 'annotations'];
