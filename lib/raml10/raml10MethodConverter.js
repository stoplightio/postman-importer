...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['responses', 'headers', 'bodies', 'method', 'parameters', 'consumes', 'usage', 'path', 'produces', 'annotations'];
	...
	static importBodies(object, converter) {
		...
		if (object.hasOwnProperty('body')) {
			for (const id in object.body) {
				...
				const body = new Body();
				body.mimeType = id;
				body.definition = value.hasOwnProperty('schema') ? converter._import(value.schema) : converter._import(value);
