...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['responses', 'headers', 'bodies', 'method', 'parameters', 'consumes', 'usage', 'path'];
		...
		const traitModel = helper.getTraitModel(this.model, this.parentResource, model.method);
		const inheritedResponses = helper.getInheritedResponses(resourceTypeModel, traitModel);
		const inheritedBodies = helper.getInheritedBodies(traitModel);
		const inheritedHeaders = helper.getInheritedHeaders(traitModel);
		const inheritedParams = helper.getInheritedParams(traitModel);
			...
			if (_.isArray(model.responses) && !_.isEmpty(model.responses)) {
				...
				for (const index in model.responses) {
					...
					if (val.hasOwnProperty('httpStatusCode') && val.httpStatusCode != 'default' &&
								...
								!inheritedResponses.includes(val.httpStatusCode) && !val.hasOwnProperty('reference')) {
						const response = {};
		...
		if (model.hasOwnProperty('headers')) {
			if (_.isArray(model.headers) && !_.isEmpty(model.headers)) {
				...
				const headers = parameterConverter.export(model.headers, inheritedHeaders);
		...
		const body = Raml10MethodConverter.exportBodies(model, raml10DefinitionConverter, inheritedBodies);
		if (!_.isEmpty(body)) ramlDef.body = body;
		
		return ramlDef;
	}
	
	static exportBodies(object, converter, inheritedBodies) {
		const body = {};
		if (object.hasOwnProperty('bodies')) {
			if (_.isArray(object.bodies) && !_.isEmpty(object.bodies)) {
				for (const index in object.bodies) {
					if (!object.bodies.hasOwnProperty(index) || inheritedBodies.includes(object.bodies[index].mimeType)) continue;
