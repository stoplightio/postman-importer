...
class Raml10MethodConverter extends Converter {
	...
	_export(model) {
		...
		if (model.hasOwnProperty('queryStrings')) {
			if (_.isArray(model.queryStrings) && !_.isEmpty(model.queryStrings)) {
				const parameterConverter = new ParameterConverter(this.model, this.parentResource, traitModels, inheritedParams, 'query');
				const queryString = parameterConverter.export(model.queryStrings, inheritedParams);
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['responses', 'headers', 'body', 'queryParameters', 'queryString', 'name', 'usage', 'is', 'securedBy', 'annotations'];
