...
class Raml10ResourceConverter extends Converter {
	...
	_export(model) {
		...
		const uriParameters = {};
    Raml10ResourceConverter.exportParameters(model, uriParameters, this.model, this.annotationPrefix, this.ramlDef);
		...
		if (model.hasOwnProperty('methods')) {
			if (_.isArray(model.methods) && !_.isEmpty(model.methods)) {
				for (const id in model.methods) {
					...
					Raml10ResourceConverter.exportParameters(model.methods[id], uriParameters, this.model, this.annotationPrefix, this.ramlDef);
				}
	...
	static exportParameters(object, uriParameters, model, annotationPrefix, ramlDef) {
