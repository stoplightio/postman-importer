...
class Raml10DefinitionConverter extends Converter {
	...
	_export(model) {
		...
		if (model.hasOwnProperty('fileReference')) {
			ramlDef.type = '!include ' + model.fileReference;
		}
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['enum', 'type', 'properties', 'items', 'schema', 'facets', 'structuredExample', 'fileTypes', 'typePropertyKind', 'strict', 'structureValue', 'examples', 'description'];
