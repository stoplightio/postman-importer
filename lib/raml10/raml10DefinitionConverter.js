...
class Raml10DefinitionConverter extends Converter {
	...
	static copyObjectFrom(object, attrIdMap, attrIdCopy) {
		const result = {};
	...
	_import(ramlDef) {
		...
		const attrIdCopyRaml = ['title', 'format', 'maxLength', 'minLength', 'exclusiveMaximum', 'exclusiveMinimum', 'maximum', 'minimum', 'definitions', 'minProperties', 'maxProperties', 'minItems', 'maxItems', 'default'];
		const attrIdCopyRaml10= _.concat(attrIdCopyRaml, ['name', 'uniqueItems', 'discriminator', 'multipleOf', 'pattern', 'displayName', 'default', 'schemaPath', 'required', 'xml', 'additionalProperties', 'minItems', 'maxItems', 'annotations', 'allowedTargets', '$ref', 'minProperties', 'maxProperties']);
		const attrIdCopyRaml08 = _.concat(attrIdCopyRaml, ['pattern']);
		...
		const model = isRaml08Version ? Raml10DefinitionConverter.copyObjectFrom(ramlDef, attrIdMap, attrIdCopyRaml08) : Raml10DefinitionConverter.copyObjectFrom(ramlDef, attrIdMap, attrIdCopyRaml10);

		if (isRaml08Version) {
			if (_.isBoolean(model.required)) delete model.required;
			if (ramlDef.hasOwnProperty('$ref')) {
				model.reference = ramlDef.$ref.replace('#/definitions/', '');
			}
		}
		...
		if (ramlDef.hasOwnProperty('properties')) {
			const required = ramlDef.hasOwnProperty('required') && _.isArray(ramlDef.required) ? ramlDef.required.filter(function (req) { return Object.keys(ramlDef.properties).includes(req); }) : [];
			...
			for (const id in ramlDef.properties) {
				...
				if (id.startsWith('/') && id.endsWith('/')) { //additionalProperties
					...
					if (!ignoreRequired && !isRaml08Version && !value.hasOwnProperty('required') || (value.hasOwnProperty('required') && value.required === true))
						required.push(id);
		...
		if (ramlDef.hasOwnProperty('items')) {
			...
			} else if (isRaml08Version && _.isArray(items)) {
				if (_.isEmpty(items)) {
					model.type = 'array';
					...
					for (const id in items) {
						...
						modelItems.push(this._import(items[id]));
					}
		...
		if (ramlDef.hasOwnProperty('example')) {
			if (typeof ramlDef.example === 'object' && !_.isArray(ramlDef.example) && !_.isEmpty(ramlDef.example)) {
				...
				const annotations = annotationConverter._import(ramlDef.example);
			...
			if (ramlDef.hasOwnProperty('structuredExample') && ramlDef.structuredExample.hasOwnProperty('strict') && !ramlDef.structuredExample.strict) {
				if (typeof model.example === 'object')
					model.example.strict = ramlDef.structuredExample.strict;
				else
					model.example = {value: model.example, strict: ramlDef.structuredExample.strict};
			}
		}
	...
	_convertSimpleType(entry, model) {
		...
		if (val.endsWith('[]')) {
			...
			if (builtinTypes.indexOf(val) < 0) {
				if (isRaml08Version && this.def && !this.def.schemas.map(schema => { return _.keys(schema)[0] }).includes(val))
					model.type = 'string';
