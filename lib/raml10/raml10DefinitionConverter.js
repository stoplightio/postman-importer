...
class Raml10DefinitionConverter extends Converter {
	...
	_import(ramlDef) {
		...
		const attrIdCopyRaml = ['title', 'format', 'maxLength', 'minLength', 'exclusiveMaximum', 'exclusiveMinimum', 'maximum', 'minimum', 'definitions'];
		...
		const attrIdCopyRaml08 = _.concat(attrIdCopyRaml, []);
		...
		if (isRaml08Version && _.isBoolean(model.required)) delete model.required;
	...
	_convertSimpleType(entry, model) {
		...
		if (val.endsWith('[]')) {
			...
			if (builtinTypes.indexOf(val) < 0) {
				if (isRaml08Version && !this.def.schemas.map(schema => { return _.keys(schema)[0] }).includes(val))
					model.type = 'string';
