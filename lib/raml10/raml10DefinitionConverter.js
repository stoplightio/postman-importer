...
class Raml10DefinitionConverter extends Converter {
	...
	static copyObjectFrom(object, attrIdMap, attrIdCopy) {
		...
		_.entries(object).map(([key, value]) => {
			if (attrIdCopy.indexOf(key) >= 0 || key.startsWith('x-')) {
	...
	_import(ramlDef) {
		...
		const attrIdSkip = ['enum', 'type', 'properties', 'items', 'schema', 'facets', 'structuredExample', 'fileTypes', 'typePropertyKind', 'strict', 'structureValue', 'examples', 'description', 'example', 'oneOf', 'allOf'];
		const attrIdCopy = ['title', 'format', 'maxLength', 'minLength', 'exclusiveMaximum', 'exclusiveMinimum', 'maximum', 'minimum', 'definitions'];
		...
		const model = isRaml08Version ? Raml10DefinitionConverter.copyObjectFrom(ramlDef, attrIdMap, attrIdCopy) : Converter.copyObjectFrom(ramlDef, attrIdMap, attrIdSkip);
