...
class Raml10DefinitionConverter extends Converter {
	...
	_import(ramlDef) {
		...
		const jsonValue = ramlDef.hasOwnProperty('typePropertyKind') ? ramlDef.typePropertyKind === 'JSON' : false;
		const inplaceValue = ramlDef.hasOwnProperty('typePropertyKind') ? ramlDef.typePropertyKind === 'INPLACE' : false;
		...
		if (ramlDef.hasOwnProperty('type')) {
			if (_.isArray(ramlDef.type) && !_.isEmpty(ramlDef.type)) {
				...
				if (typeof val === 'object') {
					...
					this._convertSimpleType(ramlDef.type, model);
				}
			}
		} else {
		...
		if (jsonValue) {
			model.jsonValue = model.reference;
