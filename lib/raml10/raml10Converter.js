...
class Raml10Converter extends Converter {
	...
	import(ramlDef) {
		...
		rootConverter.version = this.type.className;
		...
		definitionConverter.version = this.type.className;
		...
		resourceTypeConverter.version = this.type.className;
		...
		resourceConverter.version = this.type.className;
	...
	static fixInheritedProperties(model) {
		...
		for (const id in map) {
			...
			if (item.type === 'body' && userMethod.bodies) {
				...
				if (item.name && bodyMimeTypes.includes(item.name)) userMethod.bodies.splice(bodyMimeTypes.indexOf(item.name), 1);
