	...
	static fixInheritedProperties(model) {
		...
		for (const i in resources) {
			...
			if (resource.hasOwnProperty('is')) {
				for (const j in resource.is) {
					...
					Raml10Converter.mapMethodProperties(map, usedTrait.method, usedTrait.name, resource.path, 'all', is.value);
				}
			}
			for (const j in resource.methods) {
				...
				if (method.hasOwnProperty('is')) {
					for (const k in method.is) {
						...
						const usedTraitName = is.name;
						const usedTrait = traits.filter(function (trait) { return usedTraitName === trait.name; })[0];
						Raml10Converter.mapMethodProperties(map, usedTrait.method, usedTrait.name, resource.path, method.method, is.value);
					}
				}
			}
		}
		...
		for (const id in map) {
			...
			if (item.type === 'body') {
				...
				userMethod.bodies.splice(bodyMimeTypes.indexOf(item.name), 1);
			...
			} else if (item.type === 'queryParameter') {
			...
			} else if (item.type === 'uriParameter') {
				const parameterNames = userResource.parameters.map(function (parameter) { return parameter.name });
				userResource.parameters.splice(parameterNames.indexOf(item.name), 1);
