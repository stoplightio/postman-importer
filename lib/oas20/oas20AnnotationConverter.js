...
class Oas20AnnotationConverter extends Converter {
	...
	_export(model) {
		...
		for (const id in model.annotations) {
			...
			const excludedAnnotations = ['oas-body-name', 'oas-deprecated', 'oas-paths', 'oas-collectionFormat', 'oas-schema-title', 'oas-global-response-definition', 'oas-responses'];
	...
	exportAnnotation(oasDef, value) {
		const oasValidFacets = ['oas-summary', 'oas-externalDocs', 'oas-readOnly', 'oas-responses-default'];
		const annotationPrefix = oasHelper.getAnnotationPrefix;
		const name = _.includes(oasValidFacets, value.name) ? value.name.replace('oas-','') : annotationPrefix + value.name;
