...
class Oas20AnnotationConverter extends Converter {
	...
	_export(model) {
		...
		for (const id in model.annotations) {
			...
			const excludedAnnotations = ['oas-body-name', 'oas-deprecated', 'oas-responses-default'];
			if (_.includes(excludedAnnotations, name) || (name === 'oas-summary' && model.hasOwnProperty('summary'))) continue;
