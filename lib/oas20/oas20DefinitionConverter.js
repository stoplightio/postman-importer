	...
	_export(model) {
		...
		if (model.hasOwnProperty('example')) {
			...
			// } else {
				...
				if (oasDef['example'].hasOwnProperty('type') && typeof oasDef['example'].type !== 'string'){
					oasDef['example']['x-type'] = oasDef['example'].type;
					delete oasDef['example'].type;
				}
				if (oasDef['example'].hasOwnProperty('$ref') && typeof oasDef['example'].$ref !== 'string'){
					oasDef['example']['x-$ref'] = oasDef['example'].$ref;
					delete oasDef['example'].$ref;
				}
			// }
		}
		...
		if (model.hasOwnProperty('additionalProperties')) {
			if (typeof model.additionalProperties === 'object') {
				if (model.additionalProperties.hasOwnProperty('required') && !model.additionalProperties.required)
					delete model.additionalProperties.required;
		...
		if (model.hasOwnProperty('items')) {
			if (_.isArray(model.items) && model.items.length > 1) {
				const itemsDef = [];
				for (const id in model.items) {
					if (!model.items.hasOwnProperty(id)) continue;
					
					itemsDef.push(this._export(model.items[id]));
				}
				oasDef.items = itemsDef;
			} else {
		...
		this.checkForDefaultType(oasDef);
