...
class Oas20DefinitionConverter extends Converter {
	...
	_export(model) {
		...
		Oas20DefinitionConverter._convertAnnotations(model);

		if (model.hasOwnProperty('jsonValue')) {
			_.assign(model, jsonHelper.parse(model.jsonValue));
		}

		const attrIdSkip = ['name', 'fileReference', 'reference', 'properties', 'compositionType', 'schema', 'items', 'additionalProperties', 'jsonValue', 'schemaPath', 'examples', '$schema', 'id'];
	...
	_import(oasDef) {
		...
		const attrIdSkip = ['$ref', 'properties', 'allOf', 'schema', 'items', 'additionalProperties'];
		...
		if (oasDef.hasOwnProperty('title')) {
			oasDef['x-oas-schema-title'] = oasDef.title;
