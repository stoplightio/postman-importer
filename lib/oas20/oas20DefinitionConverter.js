...
class Oas20DefinitionConverter extends Converter {
	...
	_export(model) {
		...
		if (model.hasOwnProperty('reference')) {
			if (!this.def || (this.def && this.def.definitions && _.keys(this.def.definitions).includes(model.reference)))
				oasDef.$ref = model.reference.startsWith('http://') ? model.reference : '#/definitions/' + model.reference;
			else
				oasDef.type = 'string';
		}
	...
	_import(oasDef) {
		...
		const attrIdSkip = ['enum', '$ref', 'properties', 'allOf', 'schema', 'items', 'additionalProperties', 'example'];
	...
	static importExample(example) {
		...
		for (const id in model) {
			...
			const annotations = converter._import(model[id]);
