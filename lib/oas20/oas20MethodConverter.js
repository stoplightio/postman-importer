...
class Oas20MethodConverter extends Converter {
	...
	_import(oasDef) {
		...
		const attrIdSkip = ['responses', 'description', 'produces', 'parameters', 'consumes', 'security', 'tags', 'deprecated', 'summary', 'externalDocs'];
			...
			if (!_.isEmpty(oasDef.responses)) {
				...
				for (const id in oasDef.responses) {
					...
					if (value.hasOwnProperty('$ref')) { 
						...
						const annotations = this.model.annotations.filter( annotation => { return annotation.name === 'oas-responses' });
						if (!_.isEmpty(annotations)) {
							const def = annotations[0].definition;
							const index = Object.keys(def).indexOf(reference);
							const val = Object.values(def)[index];
							_.assign(response, val);
						}
						 value['x-oas-global-response-definition'] = stringsHelper.computeResourceDisplayName(value.$ref); 
					}  else {
					...
					if (id === 'default') oasDef['x-oas-responses-default'] = response;
				}
				model.responses = responses;
			}
		}

		if (oasDef.hasOwnProperty('summary')) {
			model.summary = oasDef.summary;
			oasDef['x-oas-summary'] = oasDef.summary;
		}
      ...
      if (!_.isEmpty(externalDocs)) {
        ...
        oasDef['x-oas-externalDocs'] = externalDocs;
      }
		}
		if (oasDef.hasOwnProperty('parameters')) {
			if (_.isArray(oasDef.parameters) && !_.isEmpty(oasDef.parameters)) {
				...
				for (const index in oasDef.parameters) {
					...
					let dereferencedParam = (this.dereferencedAPI) ? (this.currentMethod ? this.dereferencedAPI[this.currentMethod].parameters[index] : this.dereferencedAPI) : null;
					const val = isExternal && dereferencedParam ? dereferencedParam : oasDef.parameters[index];
		...
		if (oasDef.hasOwnProperty('deprecated')) {
			oasDef['x-oas-deprecated'] = oasDef.deprecated;
			delete oasDef.deprecated;
		}
	...
	static importResponse(source, converter) {
		const response = new Response();
		if (source.hasOwnProperty('description')) response.description = source.description;
		if (source.hasOwnProperty('headers')) {
			const headers = [];
			for (const id in source.headers) {
				const header = new Parameter();
				header.name = id;
				header.definition = converter._import(source.headers[id]);
				headers.push(header);
			}
			response.headers = headers;
		}
		if (source.hasOwnProperty('schema')) {
			const body = new Body();
			body.definition = converter._import(source.schema);
