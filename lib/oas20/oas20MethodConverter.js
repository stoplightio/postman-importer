...
class Oas20MethodConverter extends Converter {
	...
	constructor(model, dereferencedAPI, resourcePath, oasDef) {
		super(model);
	...
	_export(model) {
						...
						if (value.hasOwnProperty('reference')) {
							...
							if (value.hasOwnProperty('headers')) {
								if (_.isArray(value.headers) && !_.isEmpty(value.headers)) {
									...
									const result = parameterConverter.export(value.headers);
									for (const id in value.headers) {
										if (!value.headers.hasOwnProperty(id)) continue;

										const modelHeader = value.headers[id];
										const header = result[modelHeader.name];
										if (header.hasOwnProperty('type')) {
											Oas20DefinitionConverter._convertFromInternalType(modelHeader.definition);
											if (modelHeader.definition.hasOwnProperty('type')) header.type = modelHeader.definition.type;
											if (modelHeader.definition.hasOwnProperty('format')) header.format = modelHeader.definition.format;
							...
							if (value.hasOwnProperty('bodies')) {
								if (_.isArray(value.bodies) && !_.isEmpty(value.bodies)) {
									...
									for (const index in value.bodies) {
										...
										Oas20MethodConverter.exportExamples(val.definition, response, val.mimeType, 'examples');
										schema = definitionConverter._export(val.definition);
										if (val.definition.hasOwnProperty('internalType') && val.definition.internalType === 'file') schema.type = 'file';
										if (!val.definition.hasOwnProperty('internalType') && !val.definition.hasOwnProperty('type') && schema.hasOwnProperty('type')) delete schema.type;
										...
										else if (response.schema) response.schema = { type: 'object' };
									}
								}
							}
						}
		...
		if (model.hasOwnProperty('bodies')) {
			if (_.isArray(model.bodies) && !_.isEmpty(model.bodies)) {
				...
				if (value.definition.hasOwnProperty('example') && !parameter.schema.hasOwnProperty('example')) parameter.schema.example = value.definition.example;
		...
		if (model.hasOwnProperty('formBodies')) {
			if (_.isArray(model.formBodies) && !_.isEmpty(model.formBodies)) {
				for (const id in model.formBodies) {
					...
					if (body.mimeType && !consumes.includes(body.mimeType)) consumes.push(body.mimeType);
					if (body.definition.internalType === 'file' && !consumes.includes('multipart/form-data')) consumes.push('multipart/form-data')
					let input = {};
					const propertiesRequired = body.definition.propsRequired ? body.definition.propsRequired : [];
					let hasProperties;
					if (body.definition.hasOwnProperty('properties')) {
						input = body.definition.properties;
						hasProperties = true;
					} else {
						hasProperties = false;
						const bodyDef = body.definition;
					...
					for (const index in input) {
						...
						parameter.name = input[index].name;
						parameter.type = input[index].internalType ? input[index].internalType : input[index].type;
		...
		if (model.hasOwnProperty('securedBy') && this.oasDef && this.oasDef.securityDefinitions) {
			const security = [];
			for (const key in model.securedBy) {
				if (!model.securedBy.hasOwnProperty(key)) continue;

				const securityReq = model.securedBy[key];
				if (securityReq.name !== null && Object.keys(this.oasDef.securityDefinitions).includes(securityReq.name))
		...
		if (oasDef.hasOwnProperty('responses-default')) {
			if (oasDef.hasOwnProperty('responses')) oasDef.responses['default'] = oasDef['responses-default'];
			else oasDef.responses = {'default' : oasDef['responses-default']};
			delete oasDef['responses-default'];
		}
	...
	static exportExamples(source, target, mimeType, exampleKey) {
		if (source.hasOwnProperty(exampleKey)) {
			if (!target.examples) target.examples = {};
			target.examples[mimeType] = source[exampleKey];
			delete source[exampleKey];
		}
	}
