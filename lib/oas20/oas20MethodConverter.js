					...
					if (value.hasOwnProperty('$ref')) response.reference = value.$ref;
						...
						if (value.hasOwnProperty('headers')) {
							...
							for (const index in headers) {
								if (!headers.hasOwnProperty(index)) continue;

								const val = headers[index];
                val.name = index;
                const header = new Parameter();
								if (val.hasOwnProperty('collectionFormat')) {
									val['x-oas-collectionFormat'] = val.collectionFormat;
								}
								const converter = new ParameterConverter(this.model);
								result.push(converter._import(val));
							}
