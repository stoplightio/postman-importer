...
class Oas20MethodConverter extends Converter {
	...
	_export(model) {
		...
		let parameters = Oas20MethodConverter.exportHeaders(model, definitionConverter, traitModels, inheritedHeaders);
		...
		const queryParameters = Oas20MethodConverter.exportParameters(model, 'parameters', definitionConverter, traitModels, inheritedParams);
		if (!_.isEmpty(queryParameters)) parameters = parameters.concat(queryParameters);
		
		const queryStrings = Oas20MethodConverter.exportParameters(model, 'queryStrings', definitionConverter, traitModels, inheritedParams);
	...
	static exportParameters(object, paramsType, converter, traitModels, inheritedParams) {
		let parameters = [];
