		...
		if (oasDef.hasOwnProperty('parameters')) {
			if (_.isArray(oasDef.parameters) && !_.isEmpty(oasDef.parameters)) {
				...
				for (const index in oasDef.parameters) {
					...
					const val = (oasHelper.isFilePath(oasDef.parameters[index]) && this.dereferencedAPI) ? this.dereferencedAPI[this.currentMethod].parameters[index] : oasDef.parameters[index];
