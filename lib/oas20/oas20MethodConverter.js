...
class Oas20MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['method', 'responses', 'headers', 'bodies', 'parameters', 'is', 'path', 'produces', 'consumes', 'annotations'];
	...
	static copyObjectFrom(object, attrIdMap, attrIdSkip, annotationPrefix) {
		...
		for (const id in object) {
			...
			if (attrIdSkip.indexOf(id) < 0 && !id.startsWith(annotationPrefix)) {
