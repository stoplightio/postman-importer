...
class Oas20MethodConverter extends Converter {
	...
	_export(model) {
		...
		const attrIdSkip = ['method', 'responses', 'headers', 'bodies', 'parameters', 'is', 'path'];
		...
		const inheritedResponses = helper.getInheritedResponses(null, traitModel);
			...
			if (_.isArray(model.responses) && !_.isEmpty(model.responses)) {
				...
				for (const id in model.responses) {
					...
					if (value.hasOwnProperty('httpStatusCode')) {
						const response = {};
						if (inheritedResponses.includes(value.httpStatusCode)) {
							response['$ref'] = '#/responses/trait:' + traitModel.name + ':' + value.httpStatusCode;
		...
		if (model.hasOwnProperty('headers')) {
			if (_.isArray(model.headers) && !_.isEmpty(model.headers)) {
				for (const id in model.headers) {
					...
					let parameter;
					if (inheritedHeaders.includes(value.name)) {
						parameter = { $ref: '#/parameters/trait:' + traitModel.name + ':' + value.name };
					} else {
						parameter = Object.assign({}, oas20DefinitionConverter._export(value.definition));
		...
		if (model.hasOwnProperty('parameters')) {
			if (_.isArray(model.parameters) && !_.isEmpty(model.parameters)) {
				for (const id in model.parameters) {
					...
					let parameter;
					if (inheritedParams.includes(value.name)) {
						parameter = { $ref: '#/parameters/trait:' + traitModel.name + ':' + value.name };
					} else if (value.hasOwnProperty('reference')) {
