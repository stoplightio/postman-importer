...
class Oas20ResourceConverter extends Converter {
	...
	constructor(model, dereferencedAPI, oasDef) {
		super(model);
	...
	export(models) {
		...
		models.map(model => {
			const parents = Oas20ResourceConverter.getParents(model.path, models);
	...
	_export(model) {
		...
		if (model.hasOwnProperty('methods')) {
			if (_.isArray(model.methods) && !_.isEmpty(model.methods)) {
				const methodConverter = new Oas20MethodConverter(this.model, model.path, null, this.oasDef);
	...
	static getParents(path, models) {
		const parentAbsolutePath = Oas20ResourceConverter.getParentAbsolutePath(path);
		let parents = models.filter(function(model) {
