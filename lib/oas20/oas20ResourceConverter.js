...
class Oas20ResourceConverter extends Converter {
	...
	import(oasDefs) {
		...
		for (const id in oasDefs) {
			...
			if (id.startsWith('x-')) {
				annotationPaths['(oas-' + id + ')'] = oasDefs[id];
			} else {
				this.currentPath = id;
		...
		if (!_.isEmpty(annotationPaths)){
      Oas20RootConverter.importAnnotations({ 'x-annotation-oas-paths': annotationPaths }, this.model, this.model);
    }
	...
	_import(oasDef) {
		...
		if (oasDef.hasOwnProperty('parameters')) {
			if (_.isArray(oasDef.parameters) && !_.isEmpty(oasDef.parameters)) {
				...
				for (const id in oasDef.parameters) {
					...
					const value = (oasHelper.isFilePath(oasDef.parameters[id]) && this.dereferencedAPI) ? this.dereferencedAPI[this.currentPath].parameters[id] : oasDef.parameters[id];
		...
		const methodConverter = new Oas20MethodConverter(this.model, this.dereferencedAPI[this.currentPath]);
