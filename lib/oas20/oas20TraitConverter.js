...
class Oas20TraitConverter extends Converter {
	...
	_export(model) {
		...
		if (methodResult.hasOwnProperty('responses')) {
			for (const id in methodResult.responses) {
				...
				if (responseResult.hasOwnProperty('schema') || !_.isEmpty(responseResult.description))
	...
	_import(oasDef) {
		...
		const result = oas20MethodConverter._import({ parameters: oasDef.parameters	});
		if (oasDef.hasOwnProperty('responses') && !_.isEmpty(oasDef.responses)) {
			...
			for (const id in oasDef.responses) {
				...
				bodyModel.required = false;
				bodyModel.definition = oas20DefinitionConverter._import(response.schema);
