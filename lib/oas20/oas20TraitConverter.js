...
class Oas20TraitConverter extends Converter {
	...
	export(models) {
		...
		const traits = {};
		models.map(model => {
			if (model.method) traits[model.name] = this._export(model.method);
		});
	...
	_export(model) {
		...
		if (methodResult.hasOwnProperty('responses')) {
			for (const id in methodResult.responses) {
				...
				if (!responseResult.hasParams && (id !== 'default' || (responseResult.hasOwnProperty('schema') || responseResult.hasOwnProperty('headers') && !_.isEmpty(responseResult.description))))
