...
class RAML10Importer extends RAMLImporter {
		...
		if (methodBody.properties && !_.isEmpty(methodBody.properties)) {
			switch (data.mimeType) {
				case 'application/json':
					data.body = RAML10Importer._mapSchema(methodBody);
			...
			data.body = RAMLImporter.convertResourceRefToModel(RAML10Importer.convertArray(methodBody));
		}
		else if (methodBody.schema && !_.isEmpty(methodBody.schema)) {
			data.body = RAMLImporter.convertResourceRefToModel({
	...
	static _mapSchema(definition) {
		...
		return RAMLImporter.convertSchemaRefToModel(result);
	}
	...
	mapSchemas(schemData) {
		...
		for (const index in schemData) {
			...
			for (const schemaName in schemData[index]) {
				...
				sd.Name = schemaName;
				sd.Definition = RAML10Importer._mapSchema(schemData[index][schemaName]);
