...
class RAML10 extends RAML {
				...
				switch (data.mimeType) {
					...
					case 'application/x-www-form-urlencoded':
						...
						for (let j in formParams) {
							...
							this._convertRequiredToArray(param, param.name, data.body.required);
						}
						break;
					default:
				}
	...
	convertObjectProperty = function (source) {
		let target = {
			properties: {},
			type: 'object',
			required: []
		...
		for (let paramName in source.properties) {
			...
			else if (this.isFacet(param)) { //check for facets
				target.properties[paramName] = this.convertFacet(param);
			}
			else if (this.isAdditionalProperties(param)) {
				target.additionalProperties = this.convertAdditionalProperties(param);
				skipRequired = true;
			}
			else {
			...
			//add annotations
			this._addAnnotations(param, target.properties[paramName]);
			
			if (skipRequired) continue;
			
			//required
			this._convertRequiredToArray(param, paramName, target['required']);
		}
	...
	isRamlArray = function (object) {
		return _.endsWith(object, '[]');
	};
	
	convertRamlArray = function (object) {
		return {
			type: 'array',
			items: {
	...
	convertAdditionalProperties = function (definition) {
	...
	convertFacet = function (definition) {
	...
	convertFixedFacet = function (definition) {
		let result = [];
