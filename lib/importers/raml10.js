...
class RAML10 extends RAML {
	...
	mapRequestBody(methodBody) {
		...
		for (let i in methodBody) {
			...
			if (mimeType.properties && !_.isEmpty(mimeType.properties)) {
				...
				data.body = RAML.convertRefToModel(RAML10.convertArray(mimeType));
			}
			else if (mimeType.schema && !_.isEmpty(mimeType.schema)) {
				data.body = RAML.convertRefToModel({
	...
	convertObjectProperty(source) {
		...
		for (let paramName in source.properties) {
			...
			if (RAML10.isArray(param)) {
				target.properties[paramName] = RAML10.convertArray(param);
			}
			...
			for (let schemaName in schemData[i]) {
				...
				if (definition.type && definition.type != 'object') { //type
					...
					if (properties) { //type and properties
						...
						result = RAML10._convertCustomTypes(definition);
				...
				result = RAML10._convertCustomTypes(result);
				sd.Definition = RAML.convertRefToModel(result);
	...
	static _convertCustomTypes(result) {
		if (RAML10.isArray(result)) { //check for array
			//convert array
			result = RAML10.convertArray(result);
		}
		else if (RAML10.isFacet(result)) { //check for facets
			result = RAML10.convertFacet(result);
		}
		else if (RAML10.isFixedFacet(result)) {
			result = RAML10.convertFixedFacet(result);
		}
		return result;
	}
		...
		if (definition.hasOwnProperty('items')) {
			if (definition.items.hasOwnProperty('type')) {
				...
				if (RAML10.isRamlArray(items)) {
					definition.items = RAML10.convertArray(RAML10.convertRamlArray(definition.items));
				} else {
					definition.items = {};
