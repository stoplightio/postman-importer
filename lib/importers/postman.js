...
class Postman extends Importer {
	...
	static transformVariableFormat = function (val) {
		if (!val) return null;
		return val.replace(/\{\{(.*)\}\}/i, '<<$1>>');
	};
	
	static parseQuery = function (qstr) {
		let query = {};
	...
	_mapRequestBody = function (requestData) {
	...
	static mapConsumes = function (mode) {
		let consumes = [];
		switch (mode) {
			case 'urlencoded':
	...
	static mapEndpointGroup = function (folder) {
		return {
			name: folder.name,
			items: folder.order
		};
	...
	_mergeEndpointHeaders = function (endpoints) {
	...
	_mergeEndpointQueryString = function (endpoints) {
		return jsonHelper.stringify({
			type: 'object',
			properties: endpoints.reduce(function (result, endpoint) {
				return _.merge(result,
	...
	_mergeEndpointGroups = function (endpoints) {
	...
	_mergeEndpoints = function (endpoints) {
		let self = this;
