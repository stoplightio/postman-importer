...
//for now, if 'application/json' exist in supported type, use that
	...
	_createExtensions() {
		...
		if (definitions) {
			let definitionExtensions = {};
			for (let i in definitions) {
				if (!definitions.hasOwnProperty(i)) continue;
				let properties = definitions[i].properties;
				for (let j in properties) {
					if (!properties.hasOwnProperty(j)) continue;
					let extensions = Swagger._getExtensionsFrom(properties[j]);
					if (!_.isEmpty(extensions)) definitionExtensions[[i, j]] = extensions;
				}
			}
			if (!_.isEmpty(definitionExtensions)) {
				this.project.definitionExtensions = {};
				this.project.definitionExtensions = definitionExtensions;
			}
		}
	}
