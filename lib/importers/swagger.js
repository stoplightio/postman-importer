...
class Swagger extends Importer {
	...
	static needDeReferenced = function (param) {
		if (!param || !param.$ref) {
			return false;
		}
	...
	static mapExample = function (data, target) {
		if (data.example) {
	...
	_mapSchema = function (schemaDefinitions) {
		let result = [];
	...
	_mapQueryString = function (params, skipParameterRefs) {
	...
	_mapURIParams = function (params, resolvedParameters) {
		let pathParams = {type: 'object', properties: {}, required: []};
		for (let i in params) {
			if (!params.hasOwnProperty(i)) continue;
			let param = params[i];
			if (Swagger.needDeReferenced(param) && resolvedParameters) {
	...
	_mapRequestBody = function (params, resolvedParams) {
		if (_.isEmpty(params) ||
	...
	_mapResponseBody = function (responses, skipParameterRefs, resolvedResponses, $refs) {
		let data = [];
	...
	_mapResponseDescription = function (responseBody, description, res) {
		res.description = jsonHelper.stringify(description || responseBody.description || '');
	};
	
	_mapResponseHeaders = function (responseBody, res) {
	...
	_mapResponseExample = function (responseBody, res) {
	...
	_mapRequestHeaders = function (params, skipParameterRefs) {
	...
	_parseData = function (dataOrPath, options) {
		let me = this;
	...
	_doParseData = function (dataOrPath, options, resolve, reject) {
		let me = this;
...
//for now, if 'application/json' exist in supported type, use that
	findDefaultMimeType = function (mimeTypes) {
	...
	_mapEndpointTrait = function (params, resolvedParameters) {
		let traits = [];
		for (let i in params) {
			if (!params.hasOwnProperty(i)) continue;
			let param = params[i];
	...
	_mapEndpointTraits = function (params, responses) {
		let traits = [];
	...
	_getParams = function (params, resolvedParameters, condition) {
	...
	_getResponses = function (response, resolvedResponse) {
		let result;
	...
	_mapEndpoints = function (consumes, produces) {
	...
	_mapTraits = function (parameters, responses) {
		let traits = {},
			queryParams = {},
			headerParams = {},
			formDataParams = {},
	...
	_getExtensionsFrom = function (object) {
		let result = {};
