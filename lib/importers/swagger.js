...
class Swagger extends Importer {
	...
	_mapResponseDescription(responseBody, description, res) {
		res.description = jsonHelper.stringify(description || responseBody.description || '');
	};
	
	_mapResponseHeaders(responseBody, res) {
...
//for now, if 'application/json' exist in supported type, use that
	...
	_mapEndpointTrait(params, resolvedParameters) {
		let traits = [];
		for (let i in params) {
			if (!params.hasOwnProperty(i)) continue;
			let param = params[i];
	...
	_mapEndpointTraits(params, responses) {
		let traits = [];
	...
	_getParams(params, resolvedParameters, condition) {
	...
	_mapTraits(parameters, responses) {
		let traits = {},
			queryParams = {},
			headerParams = {},
			formDataParams = {},
