...
Swagger.prototype._mapResponseBody = function(responseBody, skipParameterRefs, resolvedResponses, $refs) {
  ...
  for (var code in responseBody) {
    ...
    var needBeReferenced = needDeReferenced(responseBody[code]);

		if (needBeReferenced && resolvedResponses) {
			var schema = resolvedResponses[code].schema;
    ...
    this._mapResponseExample(needBeReferenced ? resolvedResponses[code] : responseBody[code], res);
		this._mapResponseHeaders(needBeReferenced ? resolvedResponses[code] : responseBody[code], res);
		this._mapResponseDescription(needBeReferenced ? resolvedResponses[code] : responseBody[code], description, res);
...
Swagger.prototype._getParams = function(params, resolvedParameters) {
...
Swagger.prototype._getNonUriParams = function(params, resolvedParameters) {
	if (_.isEmpty(params)) return params;
	...
	for (var id in params) {
		...
		var param = params[id];
		var deReferenced = needDeReferenced(param);
		if (deReferenced && resolvedParameters) {
			var paramName = deReferenced[deReferenced.length - 1];
			// param = resolvedParameters[id];
			param = this.data.parameters[paramName];
			if (param.$ref) {
...
Swagger.prototype._mapEndpoints = function(consumes, produces) {
  for (var path in this.data.paths) {
    ...
    var globalParams = this._getNonUriParams(methods.parameters, resolvedPathParames);
    ...
    for (var method in methods) {
      ...
      // }
			...
			var params = _.union(this._getParams(currentMethod.parameters, currentMethodResolved.parameters), globalParams);
      var c = [];
