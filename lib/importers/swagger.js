		...
		for (let name in securityDefinitions) {
			...
			switch (type) {
				...
				case 'oauth2':
					...
					if (sd.description) {
						result[type]['description'] = sd.description;
					}
...
//for now, if 'application/json' exist in supported type, use that
	...
	static _mapEndpointTraits(params, responses) {
		let traits = [];
	...
	_import() {
		...
		if (this.data.securityDefinitions) {
			this.project.Environment.SecuritySchemes = Swagger._mapSecurityDefinitions(this.data.securityDefinitions);
		}
