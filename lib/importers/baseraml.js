...
class RAMLImporter extends Importer {
		...
		for (const i in traitGroups) {
			...
			for (const k in traitGroup) {
				...
				if (trait.queryParameters) {
					const queryString = this._filterParametricTraits(this._mapQueryParameters(trait.queryParameters), parametric);
					if (!_.isEmpty(queryString.properties))
						slTrait.request.queryString = queryString;
				}
				
				if (trait.headers) {
					const headers = this._filterParametricTraits(this._mapRequestHeaders(trait.headers), parametric);
	...
	_filterParametricTraits(traits, parametric) {
		const result = {
			properties: {},
			required: []
		};
	...
	static _mapExamples(object) {
		if (object.hasOwnProperty('example')) {
			...
			for (const id in examples) {
				...
				if (example.hasOwnProperty('name') && example.name) {
					object.examples[id]['x-raml-example-name'] = example.name;
				}
			}
		}
