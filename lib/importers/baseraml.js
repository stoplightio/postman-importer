...
class RAMLImporter extends Importer {
	...
	static _filterPropertiesFromTraits(params, traits, propertyName) {
		...
		for (let i in traits) {
			...
			for (let j in traits[i]) {
				if (!traits[i].hasOwnProperty(j)) continue;
				let param = traits[i][j];
				if (param.hasOwnProperty(propertyName)) {
					
					for (let k in param[propertyName]) {
						if (!param[propertyName].hasOwnProperty(k)) continue;
						let p = param[propertyName][k];
						let found = _.find(params, {name: p.name});
						if (found) {
							delete params[k];
						}
					}
	...
	_mapURIParams(uriParams) {
		...
		for (let i in uriParams) {
			...
			let description = key.displayName || key.description;
			if (description) {
