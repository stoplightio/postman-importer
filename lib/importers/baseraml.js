...
class RAML extends Importer {
	...
	static _mapQueryParameters(queryParameters) {
		...
		for (let key in queryParameters) {
			...
			queryString.properties[key] = RAML.convertRefToModel(ramlHelper.setParameterFields(qp, {}));
	...
	static convertRefToModel(object) {
		...
		if (typeof object === 'string') {
			if (RAML.isValidRefValue(object)) {
				return {
		...
		for (let id in object) {
			...
			if (isType) {
				if (jsonHelper.isJson(val)) {
					object = val;
					delete object[id];
				} else if (RAML.isValidRefValues(val)) {
					object.ref = val;
					delete object[id];
				}
			}
			if (isType && typeof val === 'string') {
				...
				if (id == 'structuredExample' || id == 'fixedFacets') { //delete garbage
					...
					if (id == 'xml' || id === 'example') { //no process xml object
						...
						object[id] = RAML.convertRefToModel(val);
					}
				}
			}
	...
	_mapEndpoint(resource, baseURI, pathParams) {
		...
		RAML._addAnnotations(resource, mResource.annotations);
