...
class RAMLImporter extends Importer {
	...
	static _mapParameters(queryParameters, traits, parameterName) {
		...
		for (const key in queryParameters) {
			...
			queryString.properties[key] = RAMLImporter.convertRefToModel(parameterFields);
	...
	static _mapTypesFormats(object, val) {
		if (val == 'date-only') {
			object.type = 'string';
			object.format = 'date';
		}
			...
			if (response.headers) {
				...
				for (const index in response.headers) {
					...
					if (!header.hasOwnProperty('type'))
						...
						RAMLImporter._mapTypesFormats(header, _.isArray(header.type)? header.type[0]: header.type);
	...
	static convertRefToModel(object) {
      ...
      let val = object[id];
			...
			if (isType && typeof val === 'string') {
				RAMLImporter._mapTypesFormats(object, val);
			}
