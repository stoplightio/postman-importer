...
class RAMLImporter extends Importer {
	...
	static isValidRefValue(value) {
		return typeof value === 'string' && ramlHelper.getRAML10ScalarTypes.indexOf(value) < 0 && value !== 'object';
	}
	
	// from type=type1 & schema=type1 to ref=type1
	static convertRefToModel(object, isSchema, isProperty) {
	...
	_mapEndpoint(resource, baseURI, pathParams) {
		...
		if (resources && resources.length > 0) {
			for (let j = 0; j < resources.length; j++) {
				this._mapEndpoint(resources[j], baseURI + resource.relativeUri, resultParams);
			}
		}
	}
