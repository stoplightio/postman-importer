...
class RAML10 extends RAML {
	...
	mapRequestBodyForm(bodyData) {
		...
		for (let i in body.properties) {
			...
			//facets
			RAML._addFacetsDeclaration(property, property);
		}
	...
	mapSchema(slSchemas) {
		...
		for (let i in slSchemas) {
			...
			let schemaId = schema.NameSpace;
			//check if schemaId contains invalid characters.
			schemaId = stringHelper.checkAndReplaceInvalidChars(schemaId, ramlHelper.getInvalidCharacters, ramlHelper.getReplacementCharacter);
