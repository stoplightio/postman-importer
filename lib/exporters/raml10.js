...
class RAML10Exporter extends RAMLExporter {
	...
	mapBody(bodyData, type, ramlDef) {
		const body = jsonHelper.parse(bodyData.body);
		let result = this.convertAllOfToModel(this.convertRefFromModel(body, false, null, ramlDef));
  ...
  static fixDateFormat(val) {
	...
	mapRequestBodyForm(bodyData, ramlDef) {
		...
		for (const i in body.properties) {
			...
			//facets
			//todo add annotation
			for (const id in property) {
				if (!property.hasOwnProperty(id)) continue;
				
				const found = RAMLExporter._createAnnotation(property, id, property[id], ramlDef);
				if (found) delete property[id];
			}
		}
	...
	mapSchema(slSchemas, ramlDef) {
		const results = {};
		for (const i in slSchemas) {
			if (!slSchemas.hasOwnProperty(i)) continue;
			const schema = slSchemas[i];
			let definition = this.convertRefFromModel(jsonHelper.parse(schema.Definition), true, null, ramlDef);
