...
class Swagger extends Exporter {
	...
	static _mapEndpointSecurity(securedByTypes, slSecuritySchemes) {
		let rsecuredBy = [];
		for (let index in securedByTypes) {
			...
			if (slSecuritySchemes.apiKey) {
				for (let index in slSecuritySchemes.apiKey) {
					if (!slSecuritySchemes.apiKey.hasOwnProperty(index)) continue;
					let current = slSecuritySchemes.apiKey[index];
					if (securedName === current.name) {
						let scopes = {};
						scopes[current.name] = scope;
						rsecuredBy.push(scopes);
				...
				case 'apiKey':
					for (let index in sd) {
						...
						if (current.hasOwnProperty('headers') && current.headers.length > 0) {
							for (let i in current.headers) {
								if (!current.headers.hasOwnProperty(i)) continue;
								
								let header = current.headers[i];
								result[current.name] = {
									name: header.name,
									type: type,
									in: 'header'
						...
						if (current.hasOwnProperty('queryString') && current.queryString.length > 0) {
							for (let i in current.queryString) {
								if (!current.queryString.hasOwnProperty(i)) continue;
								
								let header = current.queryString[i];
								result[current.name] = {
									name: header.name,
									type: type,
									in: 'query'
	...
	convertRefFromModel(object) {
		for (let id in object) {
			...
			if (id == 'allOf') {
				...
				for (let key in allOf) {
					if (!allOf.hasOwnProperty(key)) continue;
					let obj = allOf[key];
					if (typeof obj === 'object') allOf[key] = this.convertRefFromModel(obj);
					else allOf[key] = {
						'$ref': '#/definitions/' + obj
					};
				}
	...
	_mapResources(swaggerDef, env) {
		...
		if (!_.isEmpty(resources)) {
			for (let index in resources) {
				...
				for (let id in resource.annotations) {
					if (!resource.annotations.hasOwnProperty(id)) continue;
					let annotation = resource.annotations[id];
