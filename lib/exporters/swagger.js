...
class Swagger extends Exporter {
	...
	static mapRequiredProperty(headers){
	...
	_mapSchema(slSchemas) {
		...
		for (const i in slSchemas) {
			...
			const definition = this.convertRefFromModel(jsonHelper.parse(schema.Definition), true);
	...
	convertRefFromModel(object, isSchema) {
		...
		for (const id in object) {
			...
			if (id == 'allOf') {
				...
				if (id === 'example' || id === 'examples') {
					...
					object[id] = this.convertRefFromModel(val, isSchema);
				}
			}
		}
		...
		if (isSchema) {
			if (object.hasOwnProperty('examples')) {
				...
				if (result.hasOwnProperty('name') && result.name) {
					result['x-raml-example-name'] = result.name;
					delete result.name;
				}
