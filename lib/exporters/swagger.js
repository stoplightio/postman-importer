...
class Swagger extends Exporter {
	...
	mapExample = function (data, target) {
	...
	_getResponseTypes = function (endpoint, defaultResponseType) {
	...
	_getRequestTypes = function (endpoint, parameters, defaultRequestType) {
		let result = [],
	...
	_constructTags = function (endpoint, env) {
	...
	_constructSwaggerMethod = function (endpoint, parameters, responses, env) {
	...
	_mapEndpointSecurity = function (securedByTypes, securityDefinitions) {
		let security = [];
	...
	_mapSecurityDefinitions = function (securityDefinitions) {
		let result = {};
	...
	_mapURIParams = function (pathParams) {
		let parameters = [];
	...
	_mapQueryString = function (queryStringParams) {
		let parameters = [];
	...
	mapResponseBody = function (res, mimeType) {
		let item = {};
	...
	_mapResponseBody = function (endpoint, env) {
		...
		for (let i in slResponses) {
			...
			// }
			let item = this.mapResponseBody(res, mimeType);
	...
	_mapRequestBody = function (slRequestBody, requestTypes) {
		...
		if (!jsonHelper.isEmptySchema(body)) {
			...
			if (slRequestBody.body.match(regex) ||
				...
				this.mapExample(slRequestBody, body);
	...
	_mapRequestHeaders = function (slHeaders) {
		let result = [];
	...
	convertRefFromModel = function convertRefFromModel(object) {
	...
	_mapEndpointTraitParameters = function (endpoint, existingParams) {
	...
	_mapEndpointTraitResponses = function (endpoint) {
		if (!endpoint.traits || !endpoint.traits.length) {
			return [];
		}
	...
	_mapEndpoints = function (swaggerDef, env) {
		...
		for (let i in endpoints) {
			...
			if (!swaggerDef.paths[endpoint.Path]) {
				let params = this._validateParameters(this._mapURIParams(endpoint.PathParams));
			...
			if (endpoint.SecuredBy) {
				let security = this._mapEndpointSecurity(endpoint.SecuredBy, this.project.Environment.SecuritySchemes);
	...
	_addPatternedObjects = function (source, target) {
	...
	_mapTraitParameters = function (traits) {
		let parameters = {};
	...
	_mapTraitResponses = function (traits) {
		let responses = {};
	...
	_mapHostAndProtocol = function (env, swaggerDef) {
