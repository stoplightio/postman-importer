...
class Swagger extends Exporter {
	...
	_mapQueryString(queryStringParams) {
		...
		for (const paramName in queryStringParams.properties) {
			...
			let param = Swagger._convertExamples(queryStringParams.properties[paramName], false);
	...
	static mapHeaderProperties(headers){
	...
	_mapRequestHeaders(slHeaders) {
		...
		if (slHeaders) {
			for (const property in slHeaders.properties) {
				...
				let param = Swagger._convertExamples(slHeaders.properties[property], false);
				param = swaggerHelper.setParameterFields(param, {});
				param.name = property;
				param.in = 'header';
	...
	convertRefFromModel(object, isSchema, isProperty) {
		if (xmlHelper.isXml(object)){
			object = {type: "object" , "x-raml-xsd-definition": object};
		}
		...
		for (let id in object) {
			...
			if (!_.isArray(object) && typeof object == 'object' && !isProperty && swaggerHelper.getSupportedSchemaFields.indexOf(id) < 0){
				object["x-raml-" + id] = val;
				delete object[id];
			}
		}
	...
	static _convertExamples(object, isSchema) {
		if (isSchema){
			...
			try {
				...
				for (const p in schema.properties) {
					...
					if (!_.find(existingParams, {name: p, in: 'query'})) {
						...
						const isParametric = this._isParametricTrait(trait);
						params.push({
	...
	_mapEndpoints(swaggerDef, env, endpoints) {
		...
		for (const i in endpoints) {
			...
			if (endpoint.hasOwnProperty('is')) {
				swaggerDef.paths[endpoint.Path][endpoint.Method]['x-raml-is'] = endpoint.is;
			}
