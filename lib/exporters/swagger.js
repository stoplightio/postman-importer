...
class Swagger extends Exporter {
	...
	mapHeaderProperties(headers){
      ...
      if (header.hasOwnProperty('description') && header.description == null) header.description = '';
		...
		if (_.isArray(object)) {
			for (const id in object) {
				...
				if ((val === 'null' || val === null)) {
					if (!object.includes('string')) object[id] = 'string';
					object.splice(id, 1);
				}
			}
		}
	...
	_mapHostAndProtocol(env, swaggerDef) {
		...
		if (Array.isArray(env.Protocols) && !_.isEmpty(env.Protocols)) {
			...
			const scheme = hostUrl.protocol.split(':')[0];
			if (acceptedSchemes.includes(scheme)) swaggerDef.schemes = [scheme];
		} else {
			delete swaggerDef.schemes;
		}
	}
	
	_mapHostAnnotations(env, swaggerDef) {
		for (const id in env) {
			if (!env.hasOwnProperty(id)) continue;
			
			if (id.startsWith('x-annotation-')) swaggerDef[id] = env[id];
		}
	}
