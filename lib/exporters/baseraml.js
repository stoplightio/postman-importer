...
class RAMLExporter extends Exporter {
	...
	_mapURIParams(pathParamData) {
		...
		for (let key in pathParamData.properties) {
            ...
            let prop = RAMLExporter.withoutFormatParameter(pathParamData.properties[key]);
	...
	static _addExternalDocExtension(ramlDef, ramlObject, externalDoc) {
		if (ramlObject.hasOwnProperty('externalDocs')) delete ramlObject['externalDocs'];
	...
	static _addInnerExternalDocs(ramlDef, object) {
		...
		for (let key in object) {
			if (!object.hasOwnProperty(key)) continue;
			let value = object[key];
			if (key === 'externalDocs') RAMLExporter._addExternalDocExtension(ramlDef, object, value);
