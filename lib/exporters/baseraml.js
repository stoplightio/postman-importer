...
class RAMLDefinition {
	...
	addMethod(resource, methodURIs, methodKey, method, pathParamsRef, path) {
		...
		if (methodURIs.length <= 0) {
			...
			if (!resource[currentURI]) {
				...
				for (const attrname in method.uriParameters) {
					if (!method.uriParameters.hasOwnProperty(attrname)) continue;
					if (path) {
						if (!resource.uriParameters) resource.uriParameters = {};
						if (path.indexOf('{' + attrname + '}') >= 0) {
							resource.uriParameters[attrname] = method.uriParameters[attrname];
							delete method.uriParameters[attrname];
						}
						if (_.isEmpty(resource.uriParameters)) delete resource.uriParameters;
					}
				}
			...
			this.addMethod(resource[currentURI], methodURIs, methodKey, method, pathParamsRef, path + currentURI);
		}
	}
}
...
class RAMLExporter extends Exporter {
	...
	_export() {
		...
		RAMLExporter._refactorURIParams(ramlDef);
	...
	static _refactorURIParams(ramlDef) {
		const paths = _.keys(ramlDef).filter(key => { return key.startsWith('/') })
		if (_.isEmpty(paths)) return;
		
		for (const id in paths) {
			if (!paths.hasOwnProperty(id)) continue;
			
			const path = paths[id];
			const resource = ramlDef[path];
			if (resource.uriParameters && !_.isEmpty(resource.uriParameters)) {
				for (const name in resource.uriParameters) {
					if (!resource.uriParameters.hasOwnProperty(name)) continue;
					
					if (path.indexOf('{' + name + '}') < 0) delete resource.uriParameters[name];
				}
				if (_.isEmpty(resource.uriParameters)) delete resource.uriParameters;
			}
			RAMLExporter._refactorURIParams(resource);
		}
	}
