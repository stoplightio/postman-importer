...
class RAML extends Exporter {
	...
	_mapURIParams(pathParamData) {
		...
		for (let key in pathParamData.properties) {
            ...
            let prop = RAML.withoutFormatParameter(pathParamData.properties[key]);
			...
			if (prop.items) {
				pathParams[key].items = RAML.withoutFormatParameter(prop.items);
			}
	...
	static withoutFormatParameter(obj) {
		if (obj.hasOwnProperty('format')) {
			...
			switch (obj.type) {
				case 'string':
					...
					break;
				case 'integer':
					if (['int', 'int8', 'int16', 'int32', 'int64'].indexOf(format) < 0)
						delete obj.format;
					break;
			}
		}
	...
	_export() {
		...
		for (let i in endpoints) {
			...
			for (let i in method) {
				...
				let value = method[i];
				if (typeof value === 'object') RAML._addInnerExtensions(ramlDef, value);
			}
		}
		...
		for (let i in schemas) {
			...
			let value = schemas[i];
			if (typeof value === 'object') RAML._addInnerExtensions(ramlDef, value.definition);
		}
	...
	static _addInnerExtensions(ramlDef, object) {
		...
		if (!_.isEmpty(extensions)) RAML._addExtensions(ramlDef, object, extensions);
		for (let key in innerObjects) {
			if (!innerObjects.hasOwnProperty(key)) continue;
			let obj = innerObjects[key];
			RAML._addInnerExtensions(ramlDef, obj);
		}
	}
		...
		for (let id in object) {
			...
			if (id === 'properties' && !insideProperties) {
				for (let propId in object.properties) {
					...
					if (!RAML.checkRequiredProperty(object, propId) && typeof property === 'object') {
