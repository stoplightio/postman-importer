...
function RAMLDefinition(title, env) {
  ...
  let protocols = mapProtocols(env.Protocols);
...
RAML.prototype._export = function () {
  ...
  for(let i in endpoints) {
    ...
    if (endpoint.Summary) {
		...
		let protocols = mapProtocols(endpoint.protocols);
		if (!_.isEmpty(protocols)) {
    ...
    if (_.toLower(endpoint.Method) === 'post' ||
      ...
      let mimeType = getDefaultMimeType(endpoint.Consumes, ramlDef.mediaType);
    ...
    let mimeType = getDefaultMimeType(endpoint.Produces, ramlDef.mediaType);
