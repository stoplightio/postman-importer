...
class RAMLExporter extends Exporter {
	...
	_mapNamedParams(params) {
		...
		const newParams = {};
		const convertedParams = this.convertRefFromModel(params.properties, false);
	...
	_mapResponseBody(responseData, mimeType) {
		...
		for (const i in responseData) {
			...
			if (!_.isEmpty(resBody.codes)) {
				...
				const type = mimeType;
				const body = this.mapBody(resBody, type);
	...
	convertRefFromModel(object, isTypeDeclaration, insideProperties) {
			...
			if (object.hasOwnProperty(id)) {
				...
				if (id == '$ref' && !insideProperties) {
					...
					if (id == 'ref') {
						...
						if (isTypeDeclaration) RAMLExporter._defineFacet(object, id);
						else this._defineAnnotationType(object, id);
					}
	...
	_defineAnnotationType(object, id) {
	...
	convertSchemaTitles(object, objectType) {
		for (const id in object) {
			...
			if (id === 'title' && typeof val === 'string') {
				...
				delete object[id];
				if (objectType === 'schema') this.hasSchemaTitle = true;
				else if (objectType === 'property') this.hasPropertyTitle = true;
			}
		}
	...
	_annotationsSignature(ramlDef) {
		if (this.hasTags || this.hasDeprecated || this.hasExternalDocs || this.hasInfo ||
			this.hasSummary || this.hasSchemaTitle || this.hasPropertyTitle || this.hasBodyName ||
			this.hasResponsesDefault || this.hasGlobalResponseDefinition || this.hasDefinitionName ||
			this.hasAllowEmptyValue || this.hasCollectionFormat || this.hasExclusiveMaximum || this.hasExclusiveMinimum) {
	...
	_export() {
		...
		for (const i in endpoints) {
			...
			if (_.toLower(endpoint.Method) === 'post' ||
				...
				const body = this._mapRequestBody(endpoint.Body, mimeType);
				if (!_.isEmpty(body)) {
					method.body = body;
				}
			}
			...
			const responses = this._mapResponseBody(endpoint.Responses, mimeType);
		...
		if (this.project.Traits && this.project.Traits.length > 0) {
			const traits = this._mapParametersTraits(this.project.Traits);
			if (!_.isEmpty(traits)) {
