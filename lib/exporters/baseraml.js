...
class RAMLDefinition {
	...
	addMethod = function (resource, methodURIs, methodKey, method, pathParamsRef) {
...
class RAML extends Exporter {
	...
	_mapSecurityScheme = function (slSecuritySchemes) {
	...
	_validateParam = function (params) {
	...
	_mapRequestBody = function (bodyData, mimeType) {
		let body = {};
	...
	_mapNamedParams = function (params) {
	...
	_mapResponseBody = function (responseData, mimeType) {
		let responses = {};
	...
	_mapURIParams = function (pathParamData) {
	...
	static mapProtocols = function (protocols) {
		let validProtocols = [];
	...
	_mapTextSections = function (slTexts) {
		let results = [];
	...
	convertRefFromModel = function (object) {
	...
	_addFacetsDeclaration = function (property, target) {
	...
	_mapParametersTraits = function (slTraits) {
	...
	_mapResponsesTraits = function (slTraits, mimeType) {
		let responses = {};
	...
	_mapEndpointTraits = function (slTraits, endpoint) {
		let is = [];
	...
	getDefaultMimeType = function (mimeType, defMimeType) {
	...
	_annotationsSignature = function (ramlDef) {
	...
	_export = function () {
		...
		for (let i in endpoints) {
			...
			let is = this._mapEndpointTraits(this.project.Traits, endpoint);
			if (is.length) {
				method.is = is;
			}
			...
			if (_.toLower(endpoint.Method) === 'post' ||
				...
				let mimeType = this.getDefaultMimeType(endpoint.Consumes, ramlDef.mediaType);
			...
			let mimeType = this.getDefaultMimeType(endpoint.Produces, ramlDef.mediaType);
	...
	_addTags = function (ramlDef, tags) {
	...
	_addExtensions = function (ramlDef, ramlObject, extensions) {
	...
	_unescapeYamlIncludes = function (yaml) {
	...
	convertRequiredFromProperties = function (object) {
		for (let id in object) {
			...
			if (id === 'properties') {
				for (let propId in object.properties) {
					...
					if (!this.checkRequiredProperty(object, propId)) {
	...
	checkRequiredProperty = function (object, paramName) {
