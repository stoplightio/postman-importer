...
class RAML extends Exporter {
	...
	_export() {
		...
		let definitionExtensions = this.project.definitionExtensions;
		if (!_.isEmpty(definitionExtensions)) {
			let schemas = this.project.schemas;
			for (let i in schemas) {
				if (!schemas.hasOwnProperty(i)) continue;
				let schema = schemas[i];
				let properties = schema.definition.properties;
				for (let j in properties) {
					if (!properties.hasOwnProperty(j)) continue;
					RAML._addExtensions(ramlDef, properties[j], definitionExtensions[[schema.name, j]]);
				}
			}
		}
