...
class RAMLExporter extends Exporter {
		...
		for (const id in object) {
			if (object.hasOwnProperty(id)) {
				...
				if (id == '$ref' && !insideProperties) {
					...
					else if (val.hasOwnProperty('additionalProperties')) val.type = 'object';
					if (id === 'readOnly') RAMLExporter._createAnnotation(object, id, object[id], ramlDef);
					if (val.type != 'string') {
	...
	convertSchemaTitles(object, objectType, ramlDef) {
		for (const id in object) {
	...
	_export() {
		...
		for (const i in endpoints) {
			...
			if (_.toLower(endpoint.Method) === 'post' ||
				...
				const body = this._mapRequestBody(endpoint.Body, mimeType, ramlDef);
				if (!_.isEmpty(body)) {
					method.body = body;
				}
			}
